...
'''
The MFS branch shall mean "Multiple File Support", by enabling this feature, the 
user shall able to pack a whole dictonary of files at a time.

Based on /flet branch.
...
def pil2ft(image):
    # 使用已有的 pil_to_b64 来生成合法的 base64 字符串
...
def main(page: ft.Page):
    ...
    # async 版本：打开文件选择（Encode）
    async def encode_btn_click(e):
    ...
    # async 版本：打开文件选择（Decode）
    async def decode_btn_click(e):
        global ongoing_Type, global_file
        ongoing_Type = "decode"
        files = await ft.FilePicker().pick_files(allow_multiple=False)
        if files:
    ...
    # async 版本：执行页面内的 Encode 操作（之前缺失）
    async def in_page_encode(e):
        global global_file, pil_encoded
        print("Starting encode sequence...")
        if global_file is None:
            show_invalid_alert(page)
            return
        # 同步 encode 返回 base64 字符串
        pil_encoded = encode(global_file.path)
        # 更新显示图片（dres_img 定义在 UI 区）
        try:
            dres_img.src_base64 = pil_encoded
        except Exception:
            # 如果 dres_img 尚未定义或赋值失败，忽略并仅更新页面
            pass
        page.update()

    # 存储已编码图片（异步）
    async def save_btn_click(_):
        global pil_encoded
        if pil_encoded is None:
            ...
            page.update()
            return
        await save_image(pil_encoded)

    async def save_image(img):
        # 调用新的 save_file API（返回路径或 None）
        path_ = await ft.FilePicker().save_file(file_name="encoded_image.png", allowed_extensions=["png"])
        if path_:
            Image.open(io.BytesIO(base64.b64decode(img))
                       ).save(path_, format="PNG")
            page.snack_bar = ft.SnackBar(
                content=ft.Text("Image saved as " + path_,
    ...
    async def in_page_decode(e):
        ...
        if restore_manual_cancel:
            ...
            restore_manual_cancel = False
            # 使用新的 save_file API 获取保存路径
            path_ = await ft.FilePicker().save_file(
            ...
            if path_:
                with open(path_, 'wb') as file:
                    for i in datas:
                        file.write(i)
                page.snack_bar = ft.SnackBar(
                    content=ft.Text("File restored as " +
                                    path_, color=ft.Colors.WHITE),
                    bgcolor=ft.Colors.GREEN_400,
                    duration=2000   # 2 秒后自动消失
                )
                page.snack_bar.open = True
    ...
    # 为占位图像提供必需的 src 参数，避免 Image.__init__() 报错
    dres_img = ft.Image(src="")
    ...
    # 保持 tabs 定义不变
    tabs = [
        ft.Tab(label="Encode", icon=ft.Icon(ft.Icons.UPLOAD)),
        ft.Tab(label="Decode", icon=ft.Icon(ft.Icons.DOWNLOAD))
    ]
    ...
    tabContents = [
        ...
        ft.Column(
            ...
            controls=[
                ft.Text("Encode mode"),
                ft.Button("Pick a file", on_click=encode_btn_click),
        ...
        ft.Column(
            controls=[
                ...
                ft.Text("Click the button below to select a file..."),
                ft.Button("Pick a file", on_click=decode_btn_click),
                dfs_text, dfn_text,
    ...
    tab_Ctrl = ft.Tabs(
        ...
        length=2,  # 标签数量
        expand=True,
        content=ft.Column(  # 注意：这里是 content，不是 content_controls
            expand=True,
            controls=[
                ft.TabBar(  # 标签头区域
                    tabs=tabs,  # 传入你的 tabs 列表
                    # 可以添加其他 TabBar 属性，如 indicator_color 等
                ),
                ft.TabBarView(  # 标签内容区域
                    expand=True,
                    controls=tabContents,  # 传入你的内容列表
                ),
            ],
        ),
    )
    ...
    page.add(text_header, txt_desc, tab_Ctrl)
    page.update()
